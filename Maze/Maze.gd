...
func _ready():
	randomize()
	print(Global.coinCount)
	while coinPos.size() < Global.coinCount:
		var randx = randi() % width
		var randy = randi() % height
		if not coinPos.has(Vector2(randx, randy)) and (randx + randy) != 0:
			coinPos.append(Vector2(randx, randy))
	make_maze()
...
func make_maze():
	...
	for x in range(width):
		for z in range(height):
			...
			add_child(tile)
			if coinPos.has(Vector2(x, z)):
				var coinInt = coin.instance()
				tile.add_child(coinInt)
				coinInt.translation = Vector3(0, 1, 0)
